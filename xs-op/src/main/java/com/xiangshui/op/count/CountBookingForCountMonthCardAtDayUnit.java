...
public class CountBookingForCountMonthCardAtDayUnit extends CountProcessor<Booking> {
    ...
    private Map<Long, Integer> countMapForMonthCard = new TreeMap<>();
    ...
    protected void reduce(Booking booking, CountResult countResult) {
        if (booking != null && booking.getCreate_time() != null
            ...
            if (new Integer(1).equals(booking.getMonth_card_flag())) {
                countMapForMonthCard.put(new LocalDate(booking.getCreate_time() * 1000).toDate().getTime(), countMapForMonthCard.get(new LocalDate(booking.getCreate_time() * 1000).toDate().getTime()) + 1);
            } else {
    ...
    protected void handStart(List<Booking> data, CountResult countResult) {
        do {
            ...
            countMapForMonthCard.put(localDateStart.toDate().getTime(), 0);
